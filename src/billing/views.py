...
def _kiosk_meal_calendar(camp, meal_signups, participant):
    ...
    for meal_date in camp_meal_dates(camp, included_dates):
        ...
        for meal, _label in [(MealSignup.Meal.DINNER, "Abendessen")]:
            ...
            price_rule = _meal_price_rule(camp, meal, meal_date, participant.is_child)
            meals.append(
                {
                    "meal": meal,
...
def kiosk_home(request):
    ...
    meal_calendar_days = _kiosk_meal_calendar(participant.camp, meal_signups, participant)
    today = timezone.localdate()
    tomorrow = today + timedelta(days=1)
