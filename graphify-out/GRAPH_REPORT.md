...
## Corpus Check
- 285 files · ~395,429 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2352 nodes · 6800 edges · 275 communities (136 shown, 139 thin omitted)
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 2388 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e882ee54`
...
1. `ParticipantFactory` - 190 edges
...
## Communities (275 total, 139 thin omitted)
...
Cohesion: 0.06
Nodes (95): datetime, CampFactory, ParticipantFactory, PriceRuleFactory, test_kiosk_login_form_lists_companions_but_not_children(), test_kiosk_login_form_only_lists_non_archived_participants_from_active_camp(), test_kiosk_login_form_starts_empty_and_sorts_targets_by_last_name(), test_participant_save_calculates_booked_nights() (+87 more)

### Community 2 - "Authentication and Permissions"
Cohesion: 0.08
Nodes (62): Deliver manually confirmed email outbox entries without selecting recipients., send_due_email_deliveries(), create_settlement_run(), ChargeFactory, SuperUserFactory, test_admin_can_delete_booking_and_keeps_audit_log(), test_admin_can_edit_booking_and_creates_audit_log(), test_admin_can_restore_deleted_booking_from_audit_log() (+54 more)
...
Cohesion: 0.04
Nodes (57): camp_meal_dates(), is_meal_change_locked(), meal_change_lock_message(), Return whether kiosk meal changes are closed for the requested meal date.      A, Return the user-facing message for a closed kiosk meal slot., Return the ordered meal dates that should appear in meal overviews., Return the applicable meal price rule for one person and date.      Args:, reject_shared_expense() (+49 more)
...
Cohesion: 0.10
Nodes (25): UserFactory, test_user_can_authenticate_with_email(), test_admin_archives_and_restores_participant_without_deleting_financial_data(), test_archived_participant_is_removed_from_live_camp_settlement(), test_editor_can_edit_participant(), test_editor_cannot_archive_participant(), str, test_authenticated_navigation_links_to_passkey_management_only_when_enabled() (+17 more)
...
Cohesion: 0.13
Nodes (36): CampAnnouncementForm, EmailConfigurationForm, InformationEmailForm, ManualEmailContentForm, Meta, Edit the singleton SMTP configuration without exposing its stored password., Edit the singleton SMTP configuration without exposing its stored password., SettlementEmailForm (+28 more)
...
Cohesion: 0.10
Nodes (38): PushMessage, Store a reliable, idempotent push-delivery attempt in the database outbox., Store a reliable, idempotent push-delivery attempt in the database outbox., _administrative_users(), cleanup_push_messages(), generate_scheduled_notifications(), notify_booking_link(), notify_expense_status() (+30 more)
...
Cohesion: 0.12
Nodes (40): _html_body(), _information_dedupe_key(), information_recipient_mapping(), InformationRecipient, normalize_recipient_email(), queue_information_email_batch(), queue_settlement_email_batch(), Resolve an exact participant selection into unique normalized email recipients. (+32 more)

### Community 15 - "Playwright Setup and E2E Workflows"
Cohesion: 0.09
Nodes (42): build_settlement_backup_staging(), Create export files for a settlement run under the shared backup volume., _relative_to_backup_root(), camp_settlement_csv(), camp_workbook_response(), csv_response(), _decimal_text(), _draw_page_framework() (+34 more)
...
Cohesion: 0.07
Nodes (30): Deliver one bounded outbox batch without exposing push capabilities in logs., send_due_push_messages(), str, subscription_payload(), test_admin_can_create_and_update_own_push_subscription(), test_admin_can_rename_own_push_subscription(), test_admin_can_update_own_push_subscription_categories(), test_admin_cannot_rename_foreign_or_missing_push_subscription() (+22 more)
...
Cohesion: 0.13
Nodes (22): DrinkEntryFactory, ExpenseFactory, PaymentFactory, csv_rows(), export_dataset(), test_csv_exports_escape_formula_like_text_values(), test_csv_exports_leave_regular_text_values_unchanged(), test_drinks_csv_exports_legacy_entries_and_kiosk_drink_charges() (+14 more)
...
Cohesion: 0.27
Nodes (11): EmailDeliveryResult, _is_permanent_smtp_failure(), Return whether an SMTP failure cannot recover after a configuration update., Summarize one bounded manual email outbox delivery batch., _smtp_status_code(), EmailBatch, Record one manually confirmed information or invoice delivery batch., OSError (+3 more)
...
Cohesion: 0.23
Nodes (20): _category_selection(), _device_name(), _device_payload(), _endpoint_fingerprint(), _owner_filter(), queue_test_notification(), Queue a harmless test message for one owner-controlled device., Return a stable, non-sensitive identifier for a push endpoint. (+12 more)
...
Cohesion: 0.24
Nodes (4): has_valid_recipient_email(), Return whether persisted application data contains a deliverable address., Any, str
...
Cohesion: 0.18
Nodes (8): Run the configured daily settlement backup when the scheduled time is due., run_due_daily_settlement_backup(), Migration, Migration, test_daily_backup_creates_one_daily_backup_run(), test_daily_backup_does_not_run_before_configured_time(), test_daily_backup_logs_missing_active_camp(), test_daily_backup_retries_failed_archive_with_existing_run()
...
Cohesion: 0.25
Nodes (8): PushSubscription, Store one browser push capability for an admin or participant device., Store one browser push capability for an admin or participant device., notification_settings(), Render push-device settings for the current administrative user., PushSubscription, HttpResponse, Participant
...
Cohesion: 0.50
Nodes (4): Persist the singleton configuration for automated daily settlement backups., update_daily_backup_settings(), deployment_daily_backup_settings(), Persist the daily settlement backup schedule from the Updates page.
...
## Knowledge Gaps
- **423 isolated node(s):** `SSLContext`, `HTTPError`, `BaseException`, `HTTPStatus`, `app-entrypoint.sh script` (+418 more)
...
- **Why does `datetime` connect `Camp Settlement Exporters` to `Django Admin Integration`, `Community 65`, `Authentication and Permissions`, `ASGI App Entrypoint`, `Repository Guidelines and Rules`, `Application Settings and Setup`, `Playwright E2E Integration Tests`, `Developer Bootstrap Scripts`, `E2E Playwright Environment Variables`, `Repository Documentation`, `Community 119`, `Community 54`, `Community 55`, `Initial Database Migrations`, `Camp Schema Evolution Migrations`, `Django Core Management Script`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `Participant` connect `Django Admin Integration` to `Custom Template Tag Initializer`, `Camp Settlement Exporters`, `Authentication and Permissions`, `Repository Guidelines and Rules`, `Test Data Factories`, `Kiosk Participant Utility Views`, `Application Settings and Setup`, `Playwright E2E Integration Tests`, `Community 44`, `Developer Bootstrap Scripts`, `Playwright Setup and E2E Workflows`, `App config initializers`, `Repository Documentation`, `Community 116`, `Playwright Configuration Settings`, `Initial Database Migrations`, `Community 121`, `Community 61`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `Camp` connect `Django Admin Integration` to `Custom Template Tag Initializer`, `Camp Settlement Exporters`, `Authentication and Permissions`, `Repository Guidelines and Rules`, `Test Data Factories`, `Camp and Document Exporters`, `Kiosk Participant Utility Views`, `Application Settings and Setup`, `Playwright E2E Integration Tests`, `Developer Bootstrap Scripts`, `Playwright Setup and E2E Workflows`, `Repository Documentation`, `Playwright Configuration Settings`, `Initial Database Migrations`, `Community 121`, `Developer Codex Cleanup Utilities`?**
