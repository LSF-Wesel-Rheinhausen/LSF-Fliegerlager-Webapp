...
@pytest.mark.parametrize(
    ("model", "url_name", "label"),
    [
        (MealOrder, "mealorder", b"Essensbestellungen"),
        (Shift, "shift", b"Dienste"),
        (Participant, "participant", b"Teilnehmer"),
    ],
)
def test_admin_empty_state_is_german_and_explains_next_step(client, model, url_name, label):
    client.force_login(SuperUserFactory())
    ...
    assert b"admin-empty-state__action" in response.content
