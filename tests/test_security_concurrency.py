...
def test_quick_booking_replay_is_idempotent_across_postgresql_connections(kiosk_client, monkeypatch):
    ...
    real_save = Charge.save

    def synchronize_nonce_insert(charge, *args, **kwargs):
        if charge.kiosk_confirmation_nonce is not None:
            save_barrier.wait(timeout=10)
        return real_save(charge, *args, **kwargs)

    monkeypatch.setattr(Charge, "save", synchronize_nonce_insert)
