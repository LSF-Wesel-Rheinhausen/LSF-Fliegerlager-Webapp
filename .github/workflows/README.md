...
- `docker.yml`: Baut und prueft Fork-PRs direkt unter `pull_request` ohne Schreibrechte oder Publikation. Same-Repository-PRs und `main` starten nach erfolgreichen `Tests`-Laeufen als `workflow_run`; der lesende Testjob baut exakt dessen Head-SHA. Bei Same-Repository-PRs uebergibt er die getesteten Images als kurzlebiges Artefakt; nur der getrennte Publish-Job mit `packages: write` laedt, validiert und publiziert sie, ohne PR-Code auszufuehren. Fuer `main` prueft der Publish-Job den aktuellen Branch-Head vor beweglichen Tags. App- und Update-Agent-BuildKit-Caches bleiben ueber getrennte Scopes isoliert.
