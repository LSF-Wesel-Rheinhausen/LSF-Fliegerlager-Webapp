...
if not p:
    p = Participant.objects.create(camp=c, first_name='Max', last_name='Mustermann', arrival_date=date.today(), departure_date=date.today()+timedelta(days=10))

from billing.models import Participant, Camp, ParticipantPin
...
if not p_new:
    p_new = Participant.objects.create(camp=c, first_name='Lara', last_name='Neu', arrival_date=date.today(), departure_date=date.today()+timedelta(days=10))
...
from billing.models import ShiftAssignment
